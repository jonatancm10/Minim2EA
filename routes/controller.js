...
exports.modificarEstudiant = function (req, res){
    ...
    Estudiant.update(
        ...
        {nom : req.body.nom, direccio: req.body.direccio, telefons: {casa: req.body.telefons.casa, feina: req.body.telefons.feina}, titulacio : req.body.titulacio},
        function(err, estudiant) {
            if (err)
                res.send(err);
    ...
    Estudiant.findOne( {nom : req.body.estudiant},
        function(err, estudiant) {
            if (err) {
                ...
                console.log("Usuari:", estudiant.nom, "ID:", estudiant._id, "titulacio:", estudiant.titulacio);
        ...
        Assignatura.findOne( {nom : req.body.assignatura},
            function(err, assignatura) {
                if (err) {
                    res.send(err);
                    Console.log(err);
                    ...
                    console.log("Existeix la assignatura");
                    console.log("Assignatura:", assignatura.nom, "ID:", assignatura._id);

        Estudiant.update( {_id : req.params.estudiant_id},
               {$push: {assignatures : assignatura._id}},
               function(err, estudiant) {
                   if (err)
                       res.send(err);
                   //Devuelve la asignatura actualizada
                   Estudiant.find({_id : req.params.estudiant_id},
                       function(err, estudiant) {
                       if (err)
                           res.send(err);
                           console.log("Afegit", estudiant);
                           res.json(estudiant);
                   });
               });
             }
    ...
    Estudiant.findOne( {_id : req.params.estudiant_id},
        function(err, estudiant) {
            if (err) {
                ...
                console.log("L'estudiant no existeix!");
                res.json(estudiant);
            }
            else {
                console.log("Existeix el estudiant")
                res.json(estudiant);
            }
        });
};
