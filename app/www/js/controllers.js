...
function ($scope, $stateParams) {
  ...
  $scope.message =  'Jonatan Collado Méndez';

}])

.controller('addestudiantCtrl', ['$scope', '$stateParams', '$http', '$ionicPopup','$state', function ($scope, $stateParams, $http, $ionicPopup, $state) {
  $scope.newEstudiant = {};
    ...
    $http.post('http://localhost:8080/api/estudiant', $scope.newEstudiant)
      .success(function(data) {
        ...
        var alertPopup = $ionicPopup.alert({
                          title: 'Alumne inscrit',
                          template: 'Alumne inscrit correctament'
                        })
...
.controller('addasignaturesCtrl', ['$scope', '$stateParams', '$http', '$ionicPopup','$state', function ($scope, $stateParams, $http, $ionicPopup, $state) {
  ...
  $scope.addAssignatura = function() {
    $http.post('http://localhost:8080/api/assignatura', $scope.newAssignatura)
      .success(function(data) {
        ...
        var alertPopup = $ionicPopup.alert({
                          title: 'Assignatura registrada',
                          template: 'assignatura registrada correctament'
                        })
                        $state.go("menu.assignatures")
      })
      .error(function(data) {
        console.log('Error: ' + data);
      });
...
.controller('estudiantsCtrl', ['$scope', '$stateParams', '$http', '$state', '$ionicPopup', function ($scope, $stateParams, $http, $state, $ionicPopup) {
  $scope.newEstudiant = {};
...
/////////////////////
  .controller('filtrCtrl2', ['$scope', '$stateParams','$http',
    function ($scope, $stateParams, $http) {
      $http.get('http://localhost:8080/api/estudiant').success(function(data) {
        $scope.estudiants = data;
        console.log($scope.estudiants);
      })
        .error(function(data) {
      ...
      $scope.getEstudiantsFiltrats = function(filtre2) {
        if(filtre2 == undefined)
        {
          return;
        }
        if(filtre2.nom == "")
          filtre2.nom = undefined;
        if(filtre2.titulacio == "")
          filtre2.titulacio = undefined;
          console.log("Este estudiante es  " + this.filtre2.nom);
        $http.get('http://localhost:8080/api/estudiant', {
          params: filtre2
        }).success(function(data) {
          $scope.estudiants = data;
        }).error(function (data) {
          console.log("Error en getEstudiantsFiltrats "+ data);
        });
        filtre2 = undefined;
